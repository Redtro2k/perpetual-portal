<template>
    <div class="px-4 sm:px-0">
        <div class="sm:hidden">
            ...
            <select id="question-tabs" class="block w-full rounded-md border-gray-300 text-base font-medium text-gray-900 shadow-sm focus:border-jvgreen focus:ring-jvgreen">
        ...
        <div class="hidden sm:block">
            ...
            <Link v-for="(tab, tabIdx) in tabs" :key="tab.name" :href="tab.href" :aria-current="tab.current ? 'page' : undefined" :class="[tab.current ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700', tabIdx === 0 ? 'rounded-l-lg' : '', tabIdx === tabs.length - 1 ? 'rounded-r-lg' : '', 'group relative min-w-0 flex-1 overflow-hidden bg-white py-4 px-6 text-sm font-medium text-center hover:bg-gray-50 focus:z-10']">
                <span>{{ tab.name }}</span>
                <span aria-hidden="true" :class="[tab.current ? 'bg-jvgreen' : 'bg-transparent', 'absolute inset-x-0 bottom-0 h-0.5']" />
            </Link>
            </nav>
        </div>
    </div>
...
const nav = [
  ...
  { name: 'Annoucement', href: '#', current: false },
  { name: 'News', href: '#', current: false },
]
const tabs = computed(() => {
    if(usePage().props.value.can.manage_student){
        nav.push({name: 'Subjects',  href: route('student-subject.show', usePage().props.value.user.id), current: false})
    }
    return nav;
});
</script>
