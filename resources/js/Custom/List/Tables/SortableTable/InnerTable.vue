        ...
        <table class="min-w-full divide-y divide-gray-300">
            ...
            <tbody class="divide-y divide-gray-200">
                <tr v-for="(item, key) in items" :key="item.index">
                    <td v-if="item.length == 0" class="text-jvgreen-100 text-md">No Records
                    </td>
                    <td v-else v-for="(col, index) in Object.values(item)" :key="col"
                        ...
                        <template v-else-if="getObjectKey(props.items[key], col) == 'content' || getObjectKey(props.items[key], col) == 'title'">
                            <span v-html="limitStr(col, 10)"></span>
                        </template>
                        <template v-else-if="getObjectKey(props.items[key], col) == 'question'">
                            <span v-html="col"></span>
                        </template>
                        <template v-else-if="getObjectKey(props.items[key], col) == 'checked'">
                           <span>{{col == true ? 'available' : 'due'}}</span>
                        </template>
                        <template v-else-if="getObjectKey(props.items[key], col) == 'url'">
                            <a :href="col"
                                class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium leading-4 text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-jvgreen-100 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30">
                                <EyeIcon class="h-4 w-4" /> &nbsp; Open<span class="sr-only">, {{col}}</span>
                            </a>
                        </template>
                        <template v-else-if="getObjectKey(props.items[key], col) == 'start'">
                           <span>{{moment(col).format('LLL')}}</span>
                        </template>
                        <template v-else-if="getObjectKey(props.items[key], col) == 'due'">
                           <span>{{moment(col).format('LLL')}}</span>
                        </template>
                        <template v-else>
                            <span>{{col}}</span>
                    ...
                    <template v-if="items.length != 0">
                        <td v-if="edit_link != null" class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 md:pr-0">
                            <Link v-show="item.checked == null || item.checked == true ? true : false" v-if="!props.post && props.isDeleteData == null && edit_link_with_request == null" :href="route(edit_link, item.id)" class="text-jvgreen-100 hover:text-jvgreen-300">{{link_label}}<span class="sr-only">,
                                {{item.id}}</span>
                            </Link>
                            <template v-else-if="props.edit_link_with_request != null"
                                ...
                                <Link :href="route(edit_link, item.id)" method="get" as="button" type="button" :data="edit_link_with_request" class="text-jvgreen-100 hover:text-jvgreen-300">{{link_label}}</Link>
                            </template>
                            <template v-else-if="props.isDeleteData != null">
                                <div class="flex space-x-2">
                                    <Link v-if="isEditData || isEditData != null" :href="route(props.isEditData, item.id)" class="text-jvgreen-100 hover:text-jvgreen-300">{{link_label}}
                                    </Link>
                            ...
                            <Link v-else :href="route(edit_link)" method="get" as="button" type="button" class="text-jvgreen-100 hover:text-jvgreen-300" :data="{id: item.id}">{{link_label}}<span
                        ...
                        <td v-else-if="props.isAddData != null" class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 md:pr-0">
                            ...
                            <Link :href="route(props.isAddData.links)" type="button" as="button" method="POST" :data="{item_current_id: item.id, current_id: props.isAddData.current_id}" class="text-jvgreen-100 hover:text-jvgreen-300">Add</Link>
                        </td>
